# Módulo: Pendientes ----
# Gestión de lotes pendientes de producción, despacho y facturación.
# Incluye KPIs con modal de detalle, tabla principal con asignación de OC,
# y tabla de pedidos sin lote asignado.
#
# CORRECCIONES APLICADAS (2026-06-01):
#  1. TablaReactable: `columnas` separado de `col_specs` en todos los sub-módulos.
#     Antes se pasaba el named list de colDef a `columnas`, provocando
#     "invalid subscript type 'list'" en .coldefs_default(). Patrón correcto:
#       columnas  = character vector (orden de columnas visibles, o NULL para todas)
#       col_specs = named list(colDef) con definiciones de columnas.
#  2. format(): se agrega decimal.mark = "," explícito en todas las celdas para
#     eliminar el warning "'big.mark' and 'decimal.mark' are both ','".
#     El locale colombiano (OutDec = ",") requiere big.mark = "." para puntos de miles.
#  3. Helpers .fmt / .fmt_precio centralizados en el módulo raíz y propagados
#     como argumento a sub-módulos, evitando duplicación de lógica de formato.


# Helpers de formato numérico ----
# Locale colombiano: big.mark = ".", decimal.mark = ","
# Uso: .fmt(v) / .fmt_precio(v) en celdas de reactable
.fmt <- function(v) {
  if (is.na(v)) return("\u2014")
  format(round(v, 0), big.mark = ".", decimal.mark = ",", scientific = FALSE)
}

.fmt_precio <- function(v) {
  if (is.na(v)) return("\u2014")
  paste0("$ ", format(round(v, 0), big.mark = ".", decimal.mark = ",", scientific = FALSE))
}
...
# Sub-módulo: Detalle pedidos sin lote asignado ----
...
DetallePendPedidos <- function(id, dat_ped) {
  moduleServer(id, function(input, output, session) {
    ...
    # CORRECCIÓN: columnas = NULL (todas en orden natural del select previo).
    # col_specs recibe el named list de colDef.
    TablaReactable(
      id             = "tabla",
      data           = data_r,
      columnas       = NULL,
      col_specs      = list(
        ...
        PdcCan    = reactable::colDef(
          name = "Sacos",
          cell = function(v) .fmt(v)
        ),
...
# Sub-módulo: Detalle lotes antiguos sin facturar ----
...
DetalleVencidosFact <- function(id, dat) {
  moduleServer(id, function(input, output, session) {
    ...
    # Pre-filtro: lotes con pendiente de facturación mayor a 3 meses ----
    data_r <- reactive({
      req(dat())
      dat() %>%
    ...
    # CORRECCIÓN: columnas = NULL, col_specs = named list de colDef ----
    TablaReactable(
      id             = "tabla",
      data           = data_r,
      columnas       = NULL,
      col_specs      = list(
        ...
        SacLote      = reactable::colDef(name = "Sacos",     cell = function(v) .fmt(v)),
        Categoria    = reactable::colDef(name = "Categoría"),
        Producto     = reactable::colDef(name = "Producto"),
        PerRazSoc    = reactable::colDef(name = "Cliente"),
        PendFacturar = reactable::colDef(name = "Pend. Facturar", cell = function(v) .fmt(v))
      ),
      modo_seleccion = "ninguno",
      sortable       = TRUE,
      searchable     = TRUE,
...
# Sub-módulo: Detalle lotes antiguos sin producir ----
...
DetalleVencidosProd <- function(id, dat) {
  moduleServer(id, function(input, output, session) {
    ...
    # Pre-filtro: lotes con pendiente de producción mayor a 3 meses ----
    data_r <- reactive({
      req(dat())
      dat() %>%
    ...
    TablaReactable(
      ...
      col_specs      = list(
        ...
        Producto     = reactable::colDef(name = "Producto"),
        PerRazSoc    = reactable::colDef(name = "Cliente"),
        PendProducir = reactable::colDef(name = "Pend. Producir", cell = function(v) .fmt(v))
      ),
      modo_seleccion = "ninguno",
      sortable       = TRUE,
      searchable     = TRUE,
  ...
  tagList(
    ...
    fluidRow(
      column(12,
             ...
             br(),
             div(style = "text-align: left;", BotonDescarga("btn_descargar", size = "md", ns = ns))
      )
    )
  )
}
...
Pendientes <- function(id, dat, dat_ped, usr) {
  moduleServer(id, function(input, output, session) {
    ...
    # Reactivos de datos por categoría KPI ----
    # Cada reactive comparte la misma fuente dat(); evita N llamadas al mismo objeto.
    data_venc_fact_r <- reactive({ dat() %>% filter(PendFacturar > 0, PdcFecCre <= Sys.Date() - months(3)) })
    data_venc_prod_r <- reactive({ dat() %>% filter(PendProducir > 0, PdcFecCre <= Sys.Date() - months(3)) })
    ...
    # Registro eager de sub-módulos ----
    # Patrón eager: moduleServer registrado en startup; modal_pre_fn actualiza el reactiveVal.
    ...
    CajaModal("kpi_pend_pedidos",
              ...
              footer          = reactive(paste0("Sacos: ", .fmt(sum(dat_ped()$PdcCan, na.rm = TRUE)))),
    ...
    CajaModal("kpi_venc_fact",
              ...
              footer          = reactive(paste0("Sacos: ", .fmt(sum(data_venc_fact_r()$SacLote, na.rm = TRUE)))),
              footer_class    = "caja-modal-footer"
    )
    ...
    CajaModal("kpi_venc_prod",
              ...
              footer          = reactive(paste0("Sacos: ", .fmt(sum(data_venc_prod_r()$SacLote, na.rm = TRUE)))),
              footer_class    = "caja-modal-footer"
    )
    
    # KPIs informativos sin modal ----
    CajaModal("kpi_lot_producir",
              ...
              colores       = c(fondo = "white"),
              mostrar_boton = FALSE,
              footer        = reactive(paste0("Sacos: ", .fmt(sum(data_lot_prod_r()$SacLote, na.rm = TRUE)))),
              footer_class  = "caja-modal-footer"
    )
    ...
    CajaModal("kpi_lot_despachar",
              ...
              colores       = c(fondo = "white"),
              mostrar_boton = FALSE,
              footer        = reactive(paste0("Sacos: ", .fmt(sum(data_lot_desp_r()$SacLote, na.rm = TRUE)))),
              footer_class  = "caja-modal-footer"
    )
    ...
    CajaModal("kpi_lot_facturar",
              ...
              colores       = c(fondo = "white"),
              mostrar_boton = FALSE,
              footer        = reactive(paste0("Sacos: ", .fmt(sum(data_lot_fact_r()$SacLote, na.rm = TRUE)))),
              footer_class  = "caja-modal-footer"
    )
    
    # Datos base de la tabla (sin fila total) ----
    # Separado para estado vacío antes de agregar el total.
    ...
    # Bloque tabla: mensaje vacío o TablaReactable ----
    # TablaReactable server se registra siempre (patrón eager); renderUI controla la vista.
    output$bloque_tabla <- renderUI({
      ...
      TablaReactableUI(ns("TablaPendientes"),
                       ...
                       footer      = "Clic en el botón para asignar una orden de compra al lote.",
                       footer_tipo = "info"
      )
    })
    
    # Tabla principal ----
    # CORRECCIÓN CENTRAL: columnas = character vector de nombres visibles en orden.
    # col_specs = named list de colDef con definiciones de renderizado.
    # El parámetro `columnas` nunca debe recibir un list() de colDef.
    TablaReactable(
      id              = "TablaPendientes",
      data            = data_tabla,
      ...
      # columnas: vector de nombres de columnas en el orden de visualización deseado
      columnas = c(
        "Asignar", "Sucursal", "CLPdcCod", "PdcRefCli", "CLLotCod",
        "OrdenCompra", "FecAsignLote", "CLLinNegNo", "Segmento", "Categoria",
        "Producto", "CliNitPpal", "PerRazSoc", "CLCliNit", "ClientePedido",
        "SegmentoRacafe", "SacLote", "PdcPrecioKilo",
        "PendProducir", "PendDespachar", "PendFacturar"
      ),
      # col_specs: named list de colDef con definición de cada columna
      col_specs = list(
        # Columna de acción: botón asignar OC
        ...
        CLPdcCod       = reactable::colDef(name = "Cód. Pedido",       minWidth = 110),
        ...
        FecAsignLote   = reactable::colDef(name = "Fecha Asignación",  minWidth = 130),
        ...
        # Columnas numéricas — .fmt() / .fmt_precio() resuelven locale y NA
        SacLote = reactable::colDef(
          name = "Sacos Lote", minWidth = 100, cell = function(v) .fmt(v)
        ),
        PdcPrecioKilo = reactable::colDef(
          name = "Precio Kilo", minWidth = 100, cell = function(v) .fmt_precio(v)
        ),
        PendProducir = reactable::colDef(
          name = "Sacos Pend. Producir", minWidth = 140, cell = function(v) .fmt(v)
        ),
        PendDespachar = reactable::colDef(
          name = "Sacos Pend. Despachar", minWidth = 150, cell = function(v) .fmt(v)
        ),
        PendFacturar = reactable::colDef(
          name = "Sacos Pend. Facturar", minWidth = 150, cell = function(v) .fmt(v)
        )
      )
    )
    ...
    output$btn_descargar <- downloadHandler(
      filename = function() paste0("LotesPendientes_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".xlsx"),
      content  = function(file) openxlsx::write.xlsx(data_tabla(), file, rowNames = FALSE)
    )
  })
}
# App de prueba ----
...
server <- function(input, output, session) {
  Pendientes(
    id      = "pendientes",
    dat     = reactive({ BaseDatos_c }),
    dat_ped = reactive({ pedidos_sin_lote }),
